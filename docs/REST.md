...
| `forbidden` | 403 | нет founder scope — эндпоинт доступен только основателю |
| `not_found` | 404 | материал/вопрос/дайджест/участник по идентификатору не найден |
| `invalid_request` | 400 | неверные параметры запроса |
| `validation_error` | 400 | неверный селектор у `/founder/member` (оба/ни одного, некорректный UUID) |
...
- `limit` зажимается в диапазон `1..50` (значения за пределами подрезаются); у
  `/me/feedback` — `1..100`.
- `offset` ≥ 0.
...
### `GET /me/feedback` — ваши оценки и NPS

ВАШИ СОБСТВЕННЫЕ оценки уроков и ответы NPS (member scope) — только ваши данные,
новые первыми. `limit` зажимается в `1..100`. `meta` несёт `{limit, count}`.
Каждая строка: `knowledge_item_id` (`null` для платформенного NPS-ответа),
`metric_type` (`lesson_score` / `nps`), `score` (0..10), `comment`,
`created_at`, `updated_at`.

```bash
curl -sS "https://mcp.edgelab.space/v1/me/feedback?limit=100" \
  -H "Authorization: Bearer edgelabspace_YOUR_KEY" \
  -H "Accept: application/json"
```
...
## Founder-эндпоинты (только для основателя)

Эти эндпоинты требуют **founder scope** — доступны только member-токену, чей id
входит в список основателей. Обычный ключ участника получает `403 forbidden`.
Они отдают **агрегаты по всему сообществу** и карточку отдельного участника,
поэтому описаны здесь для полноты, но рядовому агенту недоступны. Все — `GET`.

### `GET /audience/summary`

Агрегаты аудитории: обязательный онбординг (`space_onboarding`, только
завершённые строки) — счётчики `by_ai_level` / `by_goal` / `by_role` /
`by_field` + `coverage`; вторичный `legacy_quiz` по «Точке А». Без данных
отдельных участников.

```bash
curl -sS https://mcp.edgelab.space/v1/audience/summary \
  -H "Authorization: Bearer edgelabspace_FOUNDER_KEY" \
  -H "Accept: application/json"
```

### `GET /telemetry/summary`

Агрегаты продуктовой телеметрии за окно `days` (зажимается в `1..30`, по
умолчанию 30): счётчики событий по имени/маршруту/дню + число активных
участников. Только агрегаты, никогда не сырые строки.

```bash
curl -sS "https://mcp.edgelab.space/v1/telemetry/summary?days=30" \
  -H "Authorization: Bearer edgelabspace_FOUNDER_KEY" \
  -H "Accept: application/json"
```
...
Агрегаты оценок уроков + NPS за окно `days` (зажимается в `1..90`, по умолчанию
90): средний балл, число ответов и детракторов по материалу, блок платформенного
NPS и последние комментарии (PII — только founder scope).

```bash
curl -sS "https://mcp.edgelab.space/v1/feedback/summary?days=90" \
  -H "Authorization: Bearer edgelabspace_FOUNDER_KEY" \
  -H "Accept: application/json"
```
...
Полный контекст ОДНОГО участника: личность, подписка, история уровней, ответы
онбординга, «Точка А», фидбэк, NPS-drip и 30-дневный агрегат телеметрии (никогда
не сырые строки событий). Передайте РОВНО ОДИН селектор — `tg_user_id` (числовой
Telegram id) ИЛИ `member_id` (UUID). Оба/ни одного/некорректный UUID →
`400 validation_error`; участник не найден → `404 not_found`.

```bash
curl -sS "https://mcp.edgelab.space/v1/founder/member?tg_user_id=123456789" \
  -H "Authorization: Bearer edgelabspace_FOUNDER_KEY" \
  -H "Accept: application/json"
```

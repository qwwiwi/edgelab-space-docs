...
> Те же данные доступны по обычному REST API (простые `GET`-запросы, JSON-ответы,
> без JSON-RPC) по адресу `https://mcp.edgelab.space/v1` — тот же ключ `edgelabspace_...`. Для
> скриптов и ботов REST проще; полный справочник — в
> [docs/REST.md](../../docs/REST.md). Примеры ниже используют JSON-RPC.
